...
bool IsStaticCheckSumValid()
{
	...
	for (auto& [moduleName, checksumData] : assemblyChecksumJson["modules"].items())
	{
		...
		{
			LOG_WARNING("Seems like assembly checksum don't match with version %s. Either your game isn't updated or you downloaded the wrong version of Jokebi", version.c_str());
			system("pause");
			return false;
		}
